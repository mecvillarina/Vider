...
{
    public partial class CreatePostModal : IPageBase
    {
        [Inject] public IFeedManager FeedManager { get; set; }
        [Inject] public ICreatorManager CreatorManager { get; set; }

        [Parameter] public CreateFeedPostCommand Model { get; set; } = new();
        ...
        private async Task MintAsync()
        {
            if (Validated && !IsUploadingPhoto)
            {
                try
                {
                    ...
                    await _exceptionHandler.HandlerRequestTaskAsync(() => FeedManager.CreatePostAsync(Model));
                    _appDialogService.ShowSuccess($"New post added to your feed.");
                    MudDialog.Close();
                }
