...
{
    public partial class BuyNFTModal : IPageBase
    {
        [Inject] public INFTManager NFTManager { get; set; }
        [Parameter] public NFTSellOfferItemDto SellOffer { get; set; }
        [Parameter] public BuyNFTCommand Model { get; set; } = new();
        ...
        private async Task ClaimAsync()
        {
            ...
            {
                IsProcessing = true;
                await _exceptionHandler.HandlerRequestTaskAsync(() => NFTManager.BuyNFTAsync(Model));
                await _exceptionHandler.HandlerRequestTaskAsync(() => _accountManager.GetWalletAsync());
                _appDialogService.ShowSuccess("You've successfully bought an NFT.");
                MudDialog.Close();
            }
